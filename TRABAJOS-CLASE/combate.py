...
def obtenerMatriz(img,filas, columnas,an_c,al_c):
...
def list_imagen(imag,ancho,largo,an_c,al_c):

    list=[]
    for i in range(0,largo):
        auxlis = []
        for j in range(0,ancho):
            cuadro = imag.subsurface(i*al_c,j*an_c,an_c,al_c)
...
if __name__ == '__main__':
    ...
    imagen = matrizimagen(imagen,9,6, 70 , 80)
    j = jugador(imagen)
